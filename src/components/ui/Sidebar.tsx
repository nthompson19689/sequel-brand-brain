...
/**
 * Icon lookup — keyed by module iconKey. Kept as a plain object so the
 * sidebar doesn't re-create JSX on every render cycle.
 */
...
export default function Sidebar() {
  ...
  // Filter modules by what the user has enabled, preserving registry order
  ...
  return (
    <aside className="w-60 bg-[#0F0A1A] text-[#A09CB0] flex flex-col h-screen fixed left-0 top-0 z-40">
      ...
      {/* Nav — dynamic based on user preferences */}
            ...
            {workspaceItems.map((mod) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={isActive ? "text-brand-400" : ""}>{ICONS[mod.iconKey] || ICONS.Chat}</span>
                  {mod.displayName}
                </Link>
              );
            })}
